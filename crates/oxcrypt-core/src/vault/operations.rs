...
use crate::{
    ...
    fs::name::{
        NameError, create_c9s_filename, decrypt_filename, decrypt_parent_dir_id, encrypt_filename,
        hash_dir_id,
    },
    fs::symlink::{SymlinkError, decrypt_symlink_target, encrypt_symlink_target},
    vault::cache::VaultCache,
    vault::config::{CipherCombo, VaultError, extract_master_key, validate_vault_claims},
    vault::ops::{
...
impl VaultOpContext {
    ...
    pub fn with_dir_id(mut self, dir_id: impl AsRef<str>) -> Self {
        ...
        self.dir_id = Some(if id.is_empty() {
            "<root>".to_string()
        } else {
            id.to_string()
        });
...
pub enum VaultOperationError {
    ...
    /// Key access error (async only)
    #[cfg(feature = "async")]
    #[error("Key access error: {source}")]
    KeyAccess {
        #[source]
        source: crate::crypto::keys::KeyAccessError,
    },
...
pub enum VaultWriteError {
    ...
    DirectoryNotEmpty { context: Box<VaultOpContext> },
    ...
    SameSourceAndDestination { context: Box<VaultOpContext> },
    ...
    CrossDirectoryDirSwap { context: Box<VaultOpContext> },
    ...
    fn from(err: VaultOperationError) -> Self {
        match err {
            ...
            VaultOperationError::NotAFile { path } => VaultWriteError::Io {
                ...
                context: Box::new(VaultOpContext::new().with_vault_path(path)),
            },
            ...
            VaultOperationError::NotASymlink { path } => VaultWriteError::Io {
                ...
                context: VaultOpContext::new().with_vault_path(path).into_box(),
            },
            VaultOperationError::FileContentDecryption(e) => VaultWriteError::Io {
                source: std::io::Error::other(e.to_string()),
                context: VaultOpContext::boxed(),
            },
            #[cfg(feature = "async")]
            VaultOperationError::KeyAccess { source } => VaultWriteError::Io {
...
impl VaultOperations {
    ...
    pub fn open(vault_path: &Path, password: &str) -> Result<Self, VaultError> {
        ...
        let cipher_combo = claims
            .cipher_combo()
            .expect("cipher combo already validated");
    ...
    /// Create a new VaultOperations instance with explicit cache sharing.
    ///
    /// This constructor is primarily used by `as_sync()` to share the cache
    /// between async and sync operations.
    ///
    /// # Arguments
    /// * `vault_path` - Path to the vault root directory
    /// * `master_key` - The master key for encryption/decryption
    /// * `shortening_threshold` - Maximum length for encrypted filenames before shortening
    /// * `cipher_combo` - The cipher combination used by this vault
    ...
    pub fn list_files(
        ...
        for entry in fs::read_dir(&dir_path)? {
            ...
            if path.is_dir() && is_regular_entry(&file_name) {
                continue;
            }

            // Skip other directories that aren't .c9s
            if path.is_dir() && !is_shortened_entry(&file_name) {
                continue;
            }
            ...
            } else if is_shortened_entry(&file_name) && path.is_dir() {
    ...
    pub fn list_directories(
        ...
        for entry in fs::read_dir(&dir_path)? {
            ...
            } else if path.is_dir() && is_shortened_entry(&file_name) {
    ...
    pub fn find_symlink(
        ...
        let encrypted_name = self.encrypt_filename_cached(name, directory_id.as_str())?;
    ...
    pub fn read_file(
        ...
        info!(
            decrypted_size = decrypted.content.len(),
            "File decrypted successfully"
        );
        Ok(decrypted)
    }

    /// Open a file for synchronous streaming reads.
    ///
    ...
    pub fn open_file_reader_sync(
        ...
        directory_id: &DirId,
        filename: &str,
    ) -> Result<crate::fs::streaming::VaultFileReaderSync, VaultOperationError> {
        use crate::fs::streaming::VaultFileReaderSync;
        ...
        if !paths.content_path.exists() {
            return Err(VaultOperationError::FileNotFound {
                filename: filename.to_string(),
                context: VaultOpContext::new()
                    .with_filename(filename)
                    .with_dir_id(directory_id.as_str()),
        ...
        VaultFileReaderSync::open_with_cipher(
            &paths.content_path,
            &self.master_key,
            self.core.cipher_combo(),
        )
    ...
    /// ```
    #[instrument(level = "debug", skip(self), fields(path = path.as_ref()))]
    pub fn create_directory_all(&self, path: impl AsRef<str>) -> Result<DirId, VaultWriteError> {
        let vault_path = VaultPath::new(path.as_ref());
    ...
    fn read_shortened_directory_info(
        ...
        let decrypted_name =
            decrypt_filename(&original_name, parent_dir_id.as_str(), &self.master_key)?;
    ...
    fn read_symlink_info(
        ...
        let encrypted_data =
            fs::read(&symlink_file).map_err(|_| VaultOperationError::SymlinkNotFound {
    ...
    pub fn write_file(
        ...
        let encrypted_name = self.encrypt_filename_cached(filename, dir_id.as_str())?;
    ...
    pub fn create_directory(
        ...
        let encrypted_name = self.encrypt_filename_cached(name, parent_dir_id.as_str())?;
    ...
    pub fn delete_directory(
        ...
        let files = self
            .list_files(&dir_info.directory_id)
            .map_err(|e| match e {
                VaultOperationError::Io { source, context } => VaultWriteError::Io {
                    source,
                    context: Box::new(context),
    ...
    pub fn rename_file(
        ...
        let source_info = files.iter().find(|f| f.name == old_name).ok_or_else(|| {
            VaultWriteError::FileNotFound {
        ...
        let new_encrypted_name = self.encrypt_filename_cached(new_name, dir_id.as_str())?;
    ...
    pub fn rename_directory(
        ...
        let source_info = dirs.iter().find(|d| d.name == old_name).ok_or_else(|| {
            VaultWriteError::DirectoryNotFound {
        ...
        let new_encrypted_name = self.encrypt_filename_cached(new_name, parent_dir_id.as_str())?;
        let new_is_long = new_encrypted_name.len() > self.core.shortening_threshold();
    ...
    pub fn move_file(
        ...
        let new_encrypted_name = self.encrypt_filename_cached(filename, dest_dir_id.as_str())?;
    ...
    pub fn move_and_rename_file(
        ...
        let source_info = src_files
            ...
            .ok_or_else(|| VaultWriteError::FileNotFound {
                filename: old_name.to_string(),
                context: Box::new(VaultOpContext::new().with_dir_id(src_dir_id.as_str())),
            })?;
        ...
        if dest_files.iter().any(|f| f.name == new_name) {
            return Err(VaultWriteError::FileAlreadyExists {
                filename: new_name.to_string(),
                context: Box::new(VaultOpContext::new().with_dir_id(dest_dir_id.as_str())),
            });
        }
        ...
        let new_encrypted_name = self.encrypt_filename_cached(new_name, dest_dir_id.as_str())?;
        let dest_is_long = new_encrypted_name.len() > self.core.shortening_threshold();
        ...
        if source_info.is_shortened {
            let parent = source_info.encrypted_path.parent().ok_or_else(|| {
                VaultWriteError::AtomicWriteFailed {
                    ...
                    context: Box::new(
                        VaultOpContext::new()
                            .with_vault_path(source_info.encrypted_path.display().to_string()),
                    ),
                }
            })?;
            fs::remove_dir_all(parent)?;
    ...
    fn atomic_write(path: &Path, content: &[u8]) -> Result<(), VaultWriteError> {
        let parent = path
            .parent()
            .ok_or_else(|| VaultWriteError::AtomicWriteFailed {
                reason: "No parent directory".to_string(),
                context: Box::new(
                    VaultOpContext::new().with_vault_path(path.display().to_string()),
                ),
            })?;
        ...
        temp_file
            .persist(path)
            .map_err(|e| VaultWriteError::AtomicWriteFailed {
    ...
    #[instrument(level = "debug", skip(self), fields(dir_id = %directory_id.as_str(), name = %name))]
    pub fn delete_symlink(&self, directory_id: &DirId, name: &str) -> Result<(), VaultWriteError> {
    ...
    pub fn recover_dir_id_from_backup(
        ...
        let dir_id_str = self
            .core
            .cipher_combo()
            .decrypt_dir_id_backup(&encrypted_data, &self.master_key)?;
    ...
    pub fn verify_dir_id_backup(&self, content_dir_path: &Path, expected_dir_id: &DirId) -> bool {
    ...
    pub fn recover_parent_dir_id(
        ...
        let parent_id_str =
            decrypt_parent_dir_id(&encrypted_data, dir_id.as_str(), &self.master_key)?;
    ...
    pub fn recover_directory_tree(
        &self,
    ) -> Result<Vec<RecoveredDirectoryInfo>, VaultOperationError> {
    ...
    fn recover_from_storage_directory(
        ...
        for entry in fs::read_dir(storage_path)? {
            ...
            else if is_shortened_entry(&file_name) {
    ...
    fn try_recover_directory(&self, dir_path: &Path) -> Option<RecoveredDirectoryInfo> {
        ...
        match decrypt_parent_dir_id(&encrypted_parent_id, dir_id.as_str(), &self.master_key) {
            ...
            Err(e) => {
                eprintln!(
                    "Warning: Failed to recover parent ID for directory at {}: {e}",
                    dir_path.display()
                );
                None
            }
...
#[allow(dead_code)]
pub(crate) fn debug_read_files_in_tree(
    ...
    for file in files {
        ...
        println!(
            ...
            indent, file.encrypted_size
        );

        // For text files, show content preview
        let is_text_file = Path::new(&file.name)
            .extension()
