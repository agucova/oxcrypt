...
/// Verify that lock metrics count the lock traffic generated by vault
/// operations, so profiling reports reflect real activity.
#[tokio::test]
async fn test_lock_metrics_record_lock_traffic() {
    let (_temp_dir, ops) = create_test_vault();
    let root = DirId::root();

    ops.write_file(&root, "metrics.txt", b"payload")
        .await
        .expect("Failed to write file");
    ops.read_file(&root, "metrics.txt")
        .await
        .expect("Failed to read file");
    ops.list_files(&root).await.expect("Failed to list files");

    let snapshot = ops.lock_metrics().snapshot();

    assert!(
        snapshot.fast_path_hits > 0,
        "sync fast path acquisitions should be counted: {snapshot:?}"
    );
    assert!(
        snapshot.async_acquisitions > 0,
        "async lock acquisitions should be counted: {snapshot:?}"
    );
    assert!(
        snapshot.directory_lock_requests > 0,
        "directory lock requests should be counted: {snapshot:?}"
    );
    assert!(
        snapshot.file_lock_requests > 0,
        "file lock requests should be counted: {snapshot:?}"
    );
}

/// Verify that a contended directory lock is counted as a fast path miss.
#[tokio::test]
async fn test_lock_metrics_record_fast_path_miss_under_contention() {
    let (_temp_dir, ops) = create_test_vault();
    let root = DirId::root();

    // Uncontended: the non-blocking attempt succeeds.
    assert!(ops.try_directory_read_sync(&root).is_some());

    // Contended: a writer holds the directory lock, so the attempt fails.
    let writer_guard = ops.lock_manager().directory_write(&root).await;
    assert!(ops.try_directory_read_sync(&root).is_none());
    drop(writer_guard);

    let snapshot = ops.lock_metrics().snapshot();
    assert_eq!(snapshot.fast_path_hits, 1, "{snapshot:?}");
    assert_eq!(snapshot.fast_path_misses, 1, "{snapshot:?}");
    assert_eq!(snapshot.fast_path_attempts(), 2, "{snapshot:?}");
    assert!(
        (snapshot.fast_path_hit_rate() - 0.5).abs() < f64::EPSILON,
        "hit rate should be measured against fast path attempts: {snapshot:?}"
    );
}
