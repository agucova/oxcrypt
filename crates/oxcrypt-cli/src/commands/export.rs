...
#[instrument(level = "info", name = "cmd::export", skip_all, fields(dest = %args.dest.display(), recursive = args.recursive))]
    ...
    for source in &args.sources {
        ...
        match entry_type {
            ...
            Some(EntryType::Directory) if !args.recursive => {
                anyhow::bail!(
                    "Source '{source_path}' is a directory. Use -r/--recursive to export directories."
                );
            }
            _ => {}
        }
