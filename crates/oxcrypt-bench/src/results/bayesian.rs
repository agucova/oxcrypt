...
fn compute_mean_ci(stats: &SampleStats, level: f64) -> (f64, f64) {
    ...
    let t_dist = StudentsT::new(0.0, 1.0, df)
        .expect("StudentsT creation should succeed with validated df > 0");
...
fn compute_speedup_ratio_mc(
    ...
    let t_a = StudentsT::new(stats_a.mean, stats_a.std_err, df_a)
        .expect("StudentsT creation should succeed with validated df_a > 0");
    let t_b = StudentsT::new(stats_b.mean, stats_b.std_err, df_b)
        .expect("StudentsT creation should succeed with validated df_b > 0");
